exams2json <- function(file, n = 1L, dir = ".",
  ...
  variant = "lecture", ssl.verifypeer = TRUE, fix_choice = FALSE, ...)
{
  ...
  mdtransform <- make_exercise_transform_pandoc(to = "html5")
  ...
  if(is.null(driver$transform)){
      driver$transform = mdtransform;
  }
  if(is.null(driver$write)){
      driver$write = arsnovawrite;
  }
  
  ## generate xexams
  rval <- xexams(file, n = n, dir = dir,
    driver = list(
      sweave = driver$sweave,
      read = driver$read,
...
#        active = active,
  ...
  function(exm, dir, info)
  {
   ...
   #   json$solution <- exm[[j]]$metainfo$solution
